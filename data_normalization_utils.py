...
def clean_amount(amount_str: str) -> float:
    ...
    try:
        ...
        clean_str = re.sub(r"[^\d.]", "", str(amount_str).replace(",", ""))
...
def regex_parse_invoice(text: str) -> Dict[str, str]:
    ...
    if date_match: data["invoice_date"] = normalize_date(date_match.group(1))
    
    gst_match = re.search(r'(?:gst|gstin)\s*:?\s*([0-9A-Z]{15})', text, re.I)
    if gst_match: data["gst_number"] = gst_match.group(1).strip()
    
    total_match = re.search(r'(?:total|grand total|amount due|fare)\s*:?\s*[^\d]*([\d,]+\.?\d*)', text, re.I)
    if total_match: data["total_amount"] = total_match.group(1).replace(",", "")
