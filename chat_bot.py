...
def llm_call(prompt: str) -> Tuple[str, str]:
    ...
    2. If it fails, tries Fallback Client (Groq).
    3. If it fails, tries Fallback Client (Gemini).
    """
...
def generate_human_response(query: str, invoices: List[Dict], total: float, min_inv: Any, max_inv: Any) -> Tuple[str, str]:
    ...
    if st.button("Send", type="primary") and query:
        with st.spinner("Thinking..."):
            ...
            if not filtered_invoices:
                ...
                answer, model_used = generate_human_response(query, filtered_invoices, total_val, min_inv, max_inv)
